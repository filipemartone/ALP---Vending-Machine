...
int i, opcao, codigo_adm, quantidade_adm;  //variáveis _adm só funcionam no modo administrador
...
while (true) {
    ...
    while (opcao == 0) {        //Abre menu administrador. Já exibe o inventário, o faturamento atual e possível, e dá a opção de reposição de estoque
    ...
    if (opcao > 0) {
        while (opcao > 10) {                 //verifica se o código é válido
            cout << "Codigo invalido. Digite novamente: ";
            cin >> opcao;
        }

        while (estoque[opcao - 1] == 0) {    //verifica se há o produto no estoque
        ...
        if (estoque[opcao - 1] > 0) {        //calcula o troco, atualiza o estoque e o faturamento
            ...
            cin >> pagamento;
            while (pagamento < preco[opcao - 1]) {    //Verifica se o valor inserido é menor do que o preço e avisa o usuário
                cout << "Valor insuficiente. Coloque um valor maior: ";
                cin >> pagamento;
        }

        calcula_troco(pagamento, preco[opcao - 1]);   //Chama a função que calcula o troco e exibe a mensagem final
